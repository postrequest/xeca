...
pub fn ror(val: u32, r_bits: u32, max_bits: u32) -> u32 {
    ...
    ((val & exp as u32) >> r_bits.rem_euclid(max_bits)) | (val << (max_bits-(r_bits.rem_euclid(max_bits))) & exp as u32)
}
...
pub fn shellcode_rdi_from_bytes(dll_bytes: Vec<u8>, function_name: String, user_data: String) -> Vec<u8> {
    let clear_header = true;
    let hash_function: [u8; 4];
    if !function_name.eq("") {
        let hash_function_u32 =  hash_function_name(&function_name);
...
pub fn shellcode_rdi_to_file(dll_path: &str, function_name: &str) {
    ...
    let mut shellcode_file = fs::File::create("shellcode.bin").expect("Error opening file to write shellcode");
    shellcode_file.write_all(&shellcode).expect("Could not write contents to shellcode file");
    println!("Shellcode saved to shellcode.bin");
}
